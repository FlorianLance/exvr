...
namespace SteamAudio
{
    ...
    public class SteamAudioGeometry : MonoBehaviour
    {
        ...
        int GetVerticesForTerrain(Terrain terrain, Vector3[] vertices, int offset, int localOffset)
        {
            ...
            var vertexIndex = 0;
            for (int v = 0; v < terrain.terrainData.heightmapResolution; v += s)
            {
                for (int u = 0; u < terrain.terrainData.heightmapResolution; u += s)
                {
                    float height = heights[v, u];

                    float x = position.x + (((float)u / terrain.terrainData.heightmapResolution) * 
                        terrain.terrainData.size.x);

                    float y = position.y + (height * terrain.terrainData.size.y);

                    float z = position.z + (((float)v / terrain.terrainData.heightmapResolution) * 

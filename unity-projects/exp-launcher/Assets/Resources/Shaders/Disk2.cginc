...
Varyings Vertex(Attributes input) {
	...
	Varyings o;
	o.position = mul(unity_ObjectToWorld, input.position);// input.position;
	o.color = input.color;
...
VertexOutput createParaboloidPoint(Varyings input, float u, float v) {
	VertexOutput nPoint;
...
[maxvertexcount(68)]
void Geometry(point Varyings input[1], inout TriangleStream<VertexOutput> outputStream) {
	float sqr = sqrt(2) / 2;
...
half4 Fragment(VertexOutput o) : SV_Target{
